...
这种编码方式称为补码。（更准确来说是二进制补码）

求一个数的补码即求其相反数在二进制补码下的表示：即取反再加一。

> **原码**
>
> 最高位为符号位，剩余的位数表示绝对值。但是数字运算前不太容易知道运算的结果是正数还是负数，因此原码表示的计算还需要在最后进行额外的步骤设置符号位。
>
> **1 的补码（反码）**
>
> 一个数的相反数就是这个数的二进制表示再取反，最小的负数为 10...0000。这种表示中，正数和负数的个数是对等的。
>
> **移码**
>
> 最小的负数用 00...0000 表示，最大的正数用 11...1111 表示，0 用 10...0000 表示，即通过将数字加一个偏移使其具有非负的表达形式。
